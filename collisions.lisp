...
(defun water-tile? (tile-type)
  (and (not (tall-slope? tile-type))
       (intersection (ensure-list tile-type) '(:water))))
(defun tall-slope? (tile-type)
  (intersection (ensure-list tile-type) '(:ltt :rtt :lbt :rbt)))
(defun short-slope? (tile-type)
  (intersection (ensure-list tile-type) '(:lts :rts :lbs :rbs)))
(defun wall? (tile-type)
  (intersection (ensure-list tile-type) '(:solid-player)))

(defun top-slope? (tile-type)
  (intersection (ensure-list tile-type) '(:ltt :lts :rts :rtt)))
(defun bottom-slope? (tile-type)
  (intersection (ensure-list tile-type) '(:lbt :lbs :rbs :rbt)))
(defun right-slope? (tile-type)
  (intersection (ensure-list tile-type) '(:rts :rtt :rbs :rbt)))
(defun left-slope? (tile-type)
  (intersection (ensure-list tile-type) '(:lts :ltt :lbs :lbt)))
